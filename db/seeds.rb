...
module SeedHelpers
  ...
  def self.link_to_owner(record, user:, startup_profile:)
...
def seed_founder!(email:, full_name:)
  ...
  SeedHelpers.ensure_confirmed!(user)
  user.save!
  user
end
...
def seed_milestones!(user:, startup_profile:, milestones:)
  ...
  milestones.each do |m|
    ...
    SeedHelpers.link_to_owner(milestone, user: user, startup_profile: startup_profile)

    SeedHelpers.safe_assign(milestone, {
      title: m[:title],
      name: m[:title],
...
def seed_monthly_metrics!(user:, startup_profile:, metrics:)
  ...
  metrics.each do |mm|
    ...
    SeedHelpers.link_to_owner(metric, user: user, startup_profile: startup_profile)

    SeedHelpers.safe_assign(metric, {
      month: mm[:month],
      period: mm[:month],
