  ...
  async submit(event) {
    ...
    try {
      ...
      if (res.ok && json.success) {
        this.showFlash('Startup added', 'success')
        setTimeout(() => window.location.reload(), 1000)
      } else {
        this.showFlash((json.errors || json.error || ['Failed to add startup']).join ? (json.errors || json.error).join('; ') : json.error || 'Failed to add startup', 'error')
      }
    } catch (e) {
      this.showFlash('Network error adding startup', 'error')
    }
  }
